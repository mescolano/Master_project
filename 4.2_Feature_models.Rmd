...
df <- subset(df, select = -c(Date,Year,Month))
head(df)
```
...
The normalised RMSE in the test data is 8.6%. 
...
For ToU data points the RMSE is 10%. 
...
mse.ols2 <- mean((pred.ols2 - subset(df2, Set == "Test")$mean_cons)^2)
mse.ols2
mse.ols2/y.mean

pred.ToU.ols2 <- predict.lm(ols2, newdata = df2[df2["Set"] == "Test" & df2["ToU"] == 1,])
mse.ToU.ols2 <- mean((pred.ToU.ols2 - df2[df2["Set"] == "Test" & df2["ToU"] == 1, "mean_cons"])^2)
mse.ToU.ols2
mse.ToU.ols2/y.ToU.mean

```

R-squared is 0.946 and RMSE values is just marginally better in the second model.
...
The MSE - lambda curve is quite smooth and lambda.1se is very small. Thus, regularisation is not very noticeable and the model keeps 25 coefficients out of 30 (including the intercept). All of the variables dropped are engineered features.
...
The RMS is nearly identical to that of OLS in the test data, both for the complete train dataset and considering only ToU data points. This means we have not achieved a significant improvement with this method.
...
We will start again with OLS. Adjusted R-squared has improved from 0.946 to 0.970 and normalised RMSE is 2% down to 6.5% compared to the OLS model without interactions.

```{r ols.int}
ols.int <- lm(mean_cons ~ .^2 - Holiday*. , data = subset(df2, select = -c(DateTime, sum, count, Set, ToU), Set == "Train"))
summary(ols.int)
...
Now we can apply the lasso regularised regression with second-order interactions. As we anticipated, normalised RMSE is now considerably lower compared to the OLS model with second-order interactions (2.5% and 6.5%, respectively). The prediction is also better for ToU points, with normalised RMSE equal to 3.9% (OLS with interactions yielded 8.2%).
...
Again, ridge regularisation does not improve OLS results with virtually the same RMSE.
...
It is clear that Lasso second-order is the model with the lowest error with a very significant difference compared to the rest, as we knew with the RMSE score. The vast majority of the points lie within the +/-0.1 relative error band in this model. Furthermore, the error distribution looks approximately normal, so the assumptions of the linear model hold.
...
In general all models capture the cyclic nature of the consumption data. However, simple linear models do not follow well the actual increase of consumption values in the late morning and afternoon hours of Sundays and then overestimate the same period on Mondays. This is probably due to the influence of the features constructed with the consumption of the previous day. This effect can be mitigated by including second-order interactions as it can be seen in the plots. Lasso 2nd order captures this effect much better than the remaining models while OLS and ridge 2nd order also exhibit some improvement, albeit to a lesser extent.
...
In terms of machine learning, linear feature modelling has proven to give a good fit to our test data, following its different cyclic trends (daily, weekly, seasonal). Lasso regularisation with second-order interactions is the best model we have tried with a normalised RMSE of 2.5% and provides a very good fit for a relatively simple model. Therefore, it is not necessary to explore more complex models to try to improve the fit. Including interactions has helped improve the fit of the test data especially during Sundays, where the trend of the aggregate electricity consumption during late morning and afternoon is higher than during the rest of the week.
