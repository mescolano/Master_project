...
```{r ols2.fit}
df2 <- df
df2$DateTime <- as.POSIXct(df2$DateTime, tz= "", format = "%Y-%m-%d %H:%M:%S")
...
predictions$DateTime <- as.POSIXct(predictions$DateTime, tz= "", format = "%Y-%m-%d %H:%M:%S")
head(predictions)
...
```{r plot1, echo=FALSE}
pred.plot.3 <- 
  ggplot(data = X.pred.ToU['2013-10-01/07']) + 
  geom_line(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','y.true'], colour = "True values"))+
  geom_line(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','y.ols2'], colour="OLS"))+
  geom_line(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','y.lasso'], colour="Lasso"))+
  geom_line(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','y.ridge'], colour="Ridge"))+
  geom_line(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','y.ols.int'], colour="OLS 2nd order"))+
  geom_line(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','y.lasso.int'], colour="Lasso 2nd order"))+
  geom_line(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','y.ridge.int'], colour="Ridge 2nd order"))+
  labs(title="Predicted values of different linear models",x="DateTime", y="Mean consumption [kWh/30min]")+
  scale_x_datetime(labels = date_format("%a %d/%m/%Y %H:%M"), date_breaks = "1 day")+
  scale_colour_manual(values = c("True values" = "black", "OLS" = "blue", "Lasso" = "orange","Ridge" = "green",
                                 "OLS 2nd order" = "skyblue", "Lasso 2nd order" = "darkmagenta", "Ridge 2nd order" = "forestgreen"),
...
tariff.plot.3 <- 
  ggplot(data = X.pred.ToU['2013-10-01/07']) + 
  geom_step(aes(x=index(X.pred.ToU['2013-10-01/07']), y=X.pred.ToU['2013-10-01/07','Tariff_value'], colour = "Tariff (p/kWh)"))+
  labs(title="ToU Price of Electricity",x="DateTime", y="Tariff [p/kWh]")+
  scale_x_datetime(labels = date_format("%a %d/%m/%Y %H:%M"), date_breaks = "1 day")+
  scale_colour_manual(values = c("Tariff (p/kWh)" = "black"),
                      guide = guide_legend(title = ""))

grid.arrange(pred.plot.3,tariff.plot.3, ncol = 1, nrow = 2)
```
...
This is first week of predicted data.
```{r plot4, echo=FALSE}
ggplot(data = X.pred.std['2013-10-01/07']) + 
  geom_line(aes(x=index(X.pred.std['2013-10-01/07']), y=X.pred.std['2013-10-01/07','y.true'], colour = "True values"))+
  geom_line(aes(x=index(X.pred.std['2013-10-01/07']), y=X.pred.std['2013-10-01/07','y.ols2'], colour="OLS"))+
  geom_line(aes(x=index(X.pred.std['2013-10-01/07']), y=X.pred.std['2013-10-01/07','y.lasso'], colour="Lasso"))+
  geom_line(aes(x=index(X.pred.std['2013-10-01/07']), y=X.pred.std['2013-10-01/07','y.ridge'], colour="Ridge"))+
  geom_line(aes(x=index(X.pred.std['2013-10-01/07']), y=X.pred.std['2013-10-01/07','y.ols.int'], colour="OLS 2nd order"))+
  geom_line(aes(x=index(X.pred.std['2013-10-01/07']), y=X.pred.std['2013-10-01/07','y.lasso.int'], colour="Lasso 2nd order"))+
  geom_line(aes(x=index(X.pred.std['2013-10-01/07']), y=X.pred.std['2013-10-01/07','y.ridge.int'], colour="Ridge 2nd order"))+
  labs(title="Predicted values of different linear models",x="DateTime", y="Mean consumption [kWh/30min]")+
  scale_x_datetime(labels = date_format("%a %d/%m/%Y %H:%M"), date_breaks = "1 day")+
  scale_colour_manual(values = c("True values" = "black", "OLS" = "blue", "Lasso" = "orange","Ridge" = "green",
                                 "OLS 2nd order" = "skyblue", "Lasso 2nd order" = "darkmagenta", "Ridge 2nd order" = "forestgreen"),
...
In general all models capture the cyclic nature of the consumption data. However, they do not follow well the actual increase of consumption values in the late morning and afternoon hours of Sundays and then overestimate the same period on Mondays in two of the weeks displayed here. However, this phenomenon is not always present, as in the first week of the test dataset. This is probably due to the influence of the features constructed with the consumption of the previous day. This effect has been mitigated by including second-order interactions in the OLS and Lasso models, but it is noticeable.
